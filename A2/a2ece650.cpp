        ...
        if (regex_search(line,cmatch,C)){
            ...
            if (cmatch[0] == 'E'){
                ...
                if(nums.size()){
                    ...
                    if ((street.Vert > 1) && (*std::max_element(std::begin(nums) ,std::end(nums))<Vertcomp)){
                        if(!DupeCheck(nums) && !SameCheck(nums)){  
                            ...
                            std::cerr << "Error: Duplicate edge entry\n";
                            }   
                    }
                ...
                if(nums[0]>0){
                    ...
                    std::cerr << "Error: Need to enter at least 1 Vertice\n";
                }
            }
            else if (cmatch[0] == 's'){
                nums = ExtractNumber(line,nums);
                if (street.Vert > 1 && street.Edges.size()>1){
                    ...
                    if((*std::max_element(std::begin(nums) ,std::end(nums))<Vertcomp)){
                        ...
                        if(!SP.empty()){
                            ...
                            std::cerr << "Error: No path between vertices\n";
                        } 
                    }
                    else{
                        std::cerr << "Error: Requested a path to a vertex that does not exist\n";
                    }    
                }
                else{
                    std::cerr << "Error: Not enough vertices to compute edges\n";
                ...
                std::cerr << "Error: No command match\n";
            }
        }
        else{
            continue;
